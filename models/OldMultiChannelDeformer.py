...
class CrossChannelTransformerEncoderLayer(nn.Module):
    def __init__(self, input_dimension, channels_time_dims, number_of_heads, dim_head, mlp_dim, dropout):
        ...
        inner_dim = dim_head * number_of_heads
        print(inner_dim)
        self.to_q = nn.Linear(input_dimension, inner_dim, bias=True)

        self.kv_layers = nn.ModuleList([])
        for dim in channels_time_dims:
            self.kv_layers.append(nn.ModuleList([
    ...
    def forward(self, x, other_channels_output):
        ...
        other_channels_k = []
        other_channels_v = []
        for i, (k_layer, v_layer) in zip(range(len(other_channels_output)), self.kv_layers):
            other_channel_x = other_channels_output[i]
            x_k = k_layer(other_channel_x)
            other_channels_k.append(x_k)
            x_v = v_layer(other_channel_x)
            other_channels_v.append(x_v)

        #x_agg = torch.cat(other_channels_output, dim=-1) # TODO: probably also -2

        k_agg = torch.cat(other_channels_k, dim=-2)
        v_agg = torch.cat(other_channels_v, dim=-2)
        ...
        x = x + self.sa(q, k_agg, v_agg)  # , need_weights=False
        #x = self.ln(x)
...
class Attention(nn.Module):
    def __init__(self, q_dim, heads=8, dim_head=64, dropout=0., create_heads=True): # , kv_dim
        super().__init__()
        ...
        if self.create_heads:
            # Verison without additive bias. This is only used in self attention.
