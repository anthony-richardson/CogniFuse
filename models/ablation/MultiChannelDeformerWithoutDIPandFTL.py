...
class Transformer(nn.Module):
    def __init__(self, dims, depth, heads, dim_head, mlp_dims, in_chans, emb_dims, out_dim, dropout=0.):
        super().__init__()
    ...
    def forward(self, channels_output):
        for depth_layers in self.layers:
            ...
            for i, (attn, ff, q_layer, k_layer, v_layer, _) in enumerate(depth_layers):
            ...
            for i, (_, _, _, _, _, cross_attn) in enumerate(depth_layers):
        ...
        out_embeddings = []
        for ch in channels_output:
            modality_emb = ch.view((ch.size(0), -1))
            out_embeddings.append(modality_emb)

        out = torch.cat(out_embeddings, dim=-1)
        out = self.output_ffwd(out)
        return out

    def get_modality_output_sizes(self, dims, depth, in_chans):
        return [int(dim * (0.5 ** depth)) * in_chan for dim, in_chan in zip(dims, in_chans)]
...
class MultiChannelDeformerWithoutDIPandFTL(BaseBenchmarkModel):
    @staticmethod
...
if __name__ == "__main__":
    dummy_model = MultiChannelDeformerWithoutDIPandFTL(
        num_time=[4 * 128, 6 * 128, 4 * 64, 10 * 32],
        num_chan=[16, 1, 1, 1],
        mlp_dim=[16, 16, 16, 16],
        num_kernel=[65, 4, 4, 4],
        temporal_kernel=[13, 13, 13, 13],
        emb_dim=[260, 16, 16, 16],
        depth=4,
        heads=16,
        dim_head=17,
        dropout=0.,
