    ...
    def forward(self, channels_output):
        ...
        for depth_layers in self.layers:
            ...
            for i, (attn, ff, q_layer, k_layer, v_layer, _) in enumerate(depth_layers):
                ...
                x_cg = self.pool(x)
                x_info = self.get_info(x_cg)  # (b, in_chan)
...
class MultiChannelDeformerWithoutFTL(BaseBenchmarkModel):
    @staticmethod
...
if __name__ == "__main__":
    dummy_model = MultiChannelDeformerWithoutFTL(
        num_time=[4 * 128, 6 * 128, 4 * 64, 10 * 32],
        num_chan=[16, 1, 1, 1],
        mlp_dim=[16, 16, 16, 16],
        num_kernel=[64, 4, 4, 4],
        temporal_kernel=[13, 13, 13, 13],
        emb_dim=[255, 16, 16, 16],
        depth=4,
        heads=16,
        dim_head=17,
        dropout=0.,
