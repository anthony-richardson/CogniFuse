...
class MultiChannelDeformerWithoutSFE(BaseBenchmarkModel):
    @staticmethod
    ...
    def channel_embedding(self, num_chan, out_chan):
        return nn.Sequential(
            Transpose(1, 2),
            nn.Linear(num_chan, out_chan),
            Transpose(1, 2),
    ...
    def __init__(self, *, num_time, num_chan, mlp_dim, num_kernel, temporal_kernel,
        ...
        for chan, num_kern in zip(num_chan, num_kernel):
            cnn_encoder = self.channel_embedding(num_chan=chan, out_chan=num_kern)
...
if __name__ == "__main__":
    dummy_model = MultiChannelDeformerWithoutSFE(
        num_time=[4 * 128, 6 * 128, 4 * 64, 10 * 32],
        num_chan=[16, 1, 1, 1],
        mlp_dim=[16, 16, 16, 16],
        num_kernel=[66, 4, 4, 4],
        temporal_kernel=[13, 13, 13, 13],
        emb_dim=[264, 16, 16, 16],
        depth=4,
        heads=16,
        dim_head=16,
        dropout=0.,
